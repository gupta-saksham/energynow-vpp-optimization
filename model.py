...
def build_battery_model(T,
    ...
    def soc_balance_rule(m, t):
        return m.SOC[t+1] == m.SOC[t] + (m.eta_ch*m.P_ch[t] - (1.0/m.eta_dis)*m.P_dis[t]) * m.delta_t
...
N = 7*24*4
# Extract load and prices from datasets
day_ahead_df = pd.read_csv("Day ahead.csv")
day_ahead = day_ahead_df["Germany/Luxembourg [Eur/MWh]"]/1000 # to get EUR/KWh

load_df = pd.read_csv("Load profile.csv")
