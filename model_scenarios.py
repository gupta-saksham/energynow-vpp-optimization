...
def build_battery_model(T,
                        ...
                        FCR_signal, # Gives the 15 minute time slot when FCR activation happens
                        ratios, # Stores the battery shares that are used for backup, FCR, day_ahead
    ...
    model.ratio_backup = Param(initialize=ratios[0])
    model.ratio_FCR = Param(initialize=ratios[1])
    model.ratio_FCR = Param(initialize=ratios[2])
    ...
    def soc_min_limit_rule(m, t):
        return m.SOC[t] >= fcr_upper_bound*delta_t*(1 - m.FCR_active_cumulative[t])/eta_dis + E_bat_max*m.ratio_FCR
    ...
    def fcr_power_limit(m,t):
        return m.P_FCR[t] == min(fcr_upper_bound*m.u_FCR[t]*m.FCR_active[t], E_bat_max*m.ratio_FCR)
    ...
    def sell_bin_rule(m, t):
        return m.P_sell[t] <= min((1 - m.u_buy[t]) * P_sell_max, E_bat_max*delta_t*m.ratio_day_ahead)
...
# Add ratios for the backup generator
ratio_backup = 0.2
ratio_FCR = 0.3
ratio_day_ahead = 0.5

ratios = [ratio_backup, ratio_FCR, ratio_day_ahead]
